...
class listeFormations extends Common
{
	var $Editor;
	...
	{
		$tableColumns['idFORMATION'] = array(
                    'display_text' => 'IDENTIFIANT',
                    'req' =>'req',
                    'perms' => 'EATVQSXO');
		$tableColumns['nom'] = array(
                    'display_text' => 'NOM DE LA FORMATION',
                    'req' =>'req',
                    'perms' => 'EVCTAXQSHO');
                
                $tableColumns['CF'] = array(
                    'display_text' => 'COORDINATEUR DE FORMATION',
                    'req' =>'req',
                    'perms' => 'MEVCTAXQ',
                    'join' => array(
                        'table' =>'cf',
                        'column' => 'idCF',
                        'display_mask' => "concat(cf.prenom,' ',cf.nom)",
                        'type' => 'left'),);
                
                $tableColumns['RP'] = array(
                    'display_text' => 'RESPONSABLE PEDAGOGIQUE',
                    'req' =>'req',
                    'perms' => 'MEVCTAXQ',
                    'join' => array(
                    'table' =>'rp',
                    'column' => 'idRP',
                    'display_mask' => "concat(rp.prenom,' ',rp.nom)",
                    'type' => 'left'),);

		$tableName = 'formations';
		$primaryCol = 'idFORMATION';
		$errorFun = array(&$this,'logError');
		$permissions = 'EVIDQSXHO';
		...
		$this->Editor->setConfig('addRowTitle','Ajouter une Formation');
		$this->Editor->setConfig('editRowTitle','Modifier une Formation');
		$this->Editor->setConfig('iconTitle','Op&eacute;ration');
                $this->Editor->setConfig('viewRowTitle','Informations sur la Formation ');
	}
